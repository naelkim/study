...
y
#seta()%*%v 가 아니라
#seta(1)%*%v[,1] + seta(2)%*%v[,2] + ...
#내가한건, seta1 * v[,1] + seta2 * v[,2] + ....
...
coef(lm(((y-mean(y)/sd(y)))~x0)) #y도 센터링한 경우. 절편뺴고 같네.
...
#y.hat구하고 p=1,2,3,4,5 올려가면서 비교를 해보거라. + y.bar
